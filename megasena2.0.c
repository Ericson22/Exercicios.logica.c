...
main (){
	...
	printf ("Ola, sou um programa criado com base no sistema de sorteio da mega sena\nAperte uma tecla e vamos ver o que posso fazer!\n");
	system("pause>>null");
...
for (a=0;a<LINHA;a++)//verifica se o numero repete na mesma linha  e informa onde se repete
	...
	printf ("Foram gerados %d jogos da megasena!!\nAperte uma tecla para continuar e ver quantas vezes saiu cada possivel numero.\n",LINHA);
	system("pause>>null");
   ...
   for (a=0;a<MAIORES;a++)//imprimi as duplas com maior quantidade de vezes que sairam
	...
	printf("Legal, agora quero ver os %d numeros unicos que mais sairam\n",MAIORES);
	sleep(1);
	for (a=0;a<3;a++){
		sleep(1);
		printf(".\n");
	}
	printf ("O que esta esperando?voce ja sabe o que fazer, certo?\n");
	system("pause>>null");
   ...
   for (a=0;a<MAIORES;a++){//laço para imprimir os numeros em ordem crescente
	...
	printf("Entao esses sao os %d numeros que mais aparecerao...\n",MAIORES);
	sleep(4);
  	printf("Agora pela ultima vez, por favor.\n");
  	sleep(2);
  	printf("Aperte uma tecla para verificar a quantos resultados um numero nao sai?\n");
  	system("pause>>null");
  	printf("Obrigado!!");
  	sleep(2);
	...
	printf("Muito obrigado pela ajuda, mas agora voce tambem tem otimas dicas,\n ate a proxima!\n");
	sleep(5);
	printf("O que?");
	sleep(2);
	printf("\nSe eu consigo fazer uma aposta sua?\nApenas para ver quantos numeros vc acerta no total desses %d jogos?\n",LINHA);
	sleep(6);
	printf("Aperte uma tecla que irei ve fazer %d para voce\n",APOSTA);
	system("pause>>null");
	...
	printf("Agora para que eu possa descansar aperte uma tecla.");
	system("pause>>null");
